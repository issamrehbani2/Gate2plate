...
import { airportsByCountry, flightsByAirport } from "@/lib/data"
...
export function AirportSelector() {
  ...
  // Set flight number on each keystroke; auto-fill other fields only when user finishes (onBlur)
  const handleFlightNumberChange = (value: string) => {
    setFlightInfoTemp({ ...flightInfoTemp, flightNumber: value })
  }
  ...
  useEffect(() => {
    ...
    const flightNumber = flightInfoTemp.flightNumber.trim()
    if (!flightNumber) {
    ...
    debounceRef.current = setTimeout(() => {
      ...
      const terminalToUse = selectedTerminal || selectedAirportTemp?.terminals[0]

      const updated: typeof flightInfoTemp = { ...flightInfoTemp }

      if (!updated.airline && airlineAuto) {
        updated.airline = airlineAuto
      }

      // Fill gate if missing or if it doesn't belong to the chosen terminal
      if (terminalToUse && (!updated.gate || !terminalToUse.gates.includes(updated.gate))) {
        updated.gate = pickGateFromTerminal(terminalToUse, flightNumber)
      }
  ...
  }, [flightInfoTemp.flightNumber, selectedTerminal, selectedAirportTemp])

  const handleConfirm = () => {
    if (selectedAirportTemp && flightInfoTemp.flightNumber && flightInfoTemp.gate && flightInfoTemp.boardingTime) {
      // Check boarding time is at least 60 minutes away
      const diffMinutes = minutesUntil(flightInfoTemp.boardingTime)

      if (diffMinutes < 60) {
        setConfirmError(`Cannot confirm order: boarding time is in ${diffMinutes} minute(s), which is less than 60 minutes from now.`)
        return
      }
  ...
  const availableFlights = selectedAirportTemp ? flightsByAirport[selectedAirportTemp.code] || [] : []
          ...
          <div className="space-y-2">
            ...
            {availableFlights.length > 0 ? (
              <Select
                ...
                onValueChange={(value) => {
                  const f = availableFlights.find((x) => x.flightNumber === value)
                  if (f) {
                    const term = selectedAirportTemp?.terminals.find((t) => t.id === f.terminal)
                    let newInfo: FlightInfo = { ...f }
                    if (term) {
                      setSelectedTerminal(term)
                      // Ensure gate is valid for the selected terminal; if not, pick one
                      if (!term.gates.includes(newInfo.gate)) {
                        newInfo.gate = pickGateFromTerminal(term, f.flightNumber)
                      }
                    }

                    setFlightInfoTemp(newInfo)
                    setShowAutoFields(true)
                    setLockFields(true)
                  }
                }}
          ...
          {showAutoFields && (
            <>
              ...
              <div className="space-y-2">
                ...
                <Select value={flightInfoTemp.terminal} onValueChange={handleTerminalChange} disabled={lockFields}>
